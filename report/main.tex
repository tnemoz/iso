    ...
    \section{Quantum computing preliminaries}
        ...
        \subsection{Quantum computing theory}
            ...
            \subsubsection{Quantum circuit}
                ...
                It is possible to apply a gate on a given qubit \ket{y} conditionally to the fact that another qubit is in the state \ket{1}. Such a gate is called a controlled gate, or a conditioned gate. Let us take the following circuit as an example:
                ...
                Applying \(\X\) conditioned on \ket{x} means that the gate will be applied on the parts of the state that have \ket{1} on their first qubit. Hence, the resulting state of this circuit is:
            ...
            \subsubsection{Entanglement}
                A state \ket{x} composed of \(n\) qubits is entangled whenever it is not possible to find \(n\) qubits \ket{q_1}, \ket{q_2}, \(\cdots\), \ket{q_n} such that:
    ...
    \section{A quantum recommendation system}
        ...
        \subsection{A quantum algorithm as a recommendation system}
            ...
            \subsubsection{Loading the row vector as a quantum state}
                ...
                Even though we load \ket{\hat{\mathbf{P}}_j} using its coefficients in the computational basis, we denote the quantum state in the left orthonormal basis in the SVD decomposition of \(\hat{\mathbf{P}}\), that is:
    ...
    \section{Designing a quantum circuit for a recommendation system}
        ...
        \subsection{Loading from QRAM}
            ...
            \subsubsection{Parallel execution of rotations}
                ...
                Using \(\mathbf{U}_f\), we can now have access to \ket{\theta_x}. As a reminder, since we assume \(f\) is efficiently computable classically, so is \(\mathbf{U}_f\). We now want to perform a rotation around the $y$-axis of angle $2\,\theta_x$.
                ...
                We now apply the \(\mathbf{R}_z(1)\) gate controlled on the last qubit being in state \ket{1}:
                ...
                Similarly, we apply the \(\mathbf{R}_z\left(\frac12\right)\) gate on the third qubit controlled on the last qubit being in state \ket{1}:
                ...
                \begin{figure}[ht]
                    \centering
                    \begin{quantikz}
                        \lstick{\ket{q_1}} & \gate{\mathbf{R}_y\left(2\,\theta_{\text{root}}\right)} & \gate[wires=3]{\mathbf{U}_f} & \qw & \qw & \gate[wires=3]{\mathbf{U}_f^\dagger} & \qw\\
        ...
        \subsection{Applying the Quantum Phase Estimation algorithm}
            ...
            Now that the state \ket{\hat{\mathbf{P}}_j} has been loaded from QRAM, we are to append a quantum register of size \(\lceil\log(n)\rceil\) before the quantum register in which \ket{\hat{\mathbf{P}}_j} has been loaded and to apply a unitary \(\mathbf{Q}\) to get the state:
