...
\begin{document}
    ...
    \section{Quantum computing preliminaries}
        ...
        \subsection{Quantum computing theory}
            ...
            \subsubsection{Measurement}
                ...
                Let \ket{x} be a quantum state of \(n\) qubits. We denote by \ket{k} the \(k\)\textsuperscript{th} vector of the computational basis of \(\mathbf{C}^{2^n}\). The notation \ket{k} will always be used in a non-ambiguous way. Hence, it will only be used to write a quantum state whose number of qubits is known. As such, we have:
            ...
            \subsubsection{Quantum gates and algorithms}
                ...
                \begin{definition}[\(n\)-controlled gate]
                    Let \(\mathbf{U}\) be a unitary and \(n\in\mathbf{N}\). Then it is possible to implement a gate \(\mathsf{C}^n-\mathbf{U}\) that acts on \(n+1\) qubits such that \(\mathbf{U}\) is applied to the \(n+1\)\textsuperscript{th} qubit if and only if the \(n\) other qubits are in state \ket{1} in time \(O(n)\) using \(n-1\) additional qubits. The proof and the decomposition is given in \cite{Berta}.
                    % TODO: write it in full
                \end{definition}
        ...
        \subsection{Quantum hardware considerations}
            ...
            \begin{theorem}[No-deleting theorem]
                There is no unitary \(\mathbf{U}\) that maps an arbitray state \ket{\psi} to the state \ket{0}.
            \end{theorem}

            Therefore, the no-deleting theorem still applies. Indeed, it is not possible to measure a state and then to apply \(\X\) gates accordingly.
    ...
    \section{A quantum recommendation system}
        ...
        \subsection{A quantum algorithm as a recommendation system}
            The following describes the algorithm presented in \cite{QRS}. Let \(\mathbf{P}\in\{0\,;\,1\}^{m\times n}\) a matrix that has a good rank-\(k\) approximation. That is:
            ...
            \subsubsection{Finding singular values}
                Now that we have the quantum state \(\sum_{i}\alpha_i\,\ket{v_i}\), we would like to find its singular values to get the state \(\sum_{i}\alpha_i\,\ket{v_i}\,\ket{\sigma_i}\). However, the Quantum Phase Estimation algorithm cannot be used right away, since \(\hat{\mathbf{P}}\) is not unitary.
                ...
                Hence, the task is to implement in logarithmic time \(\mathbf{W}\) and to apply the Quantum Phase Estimation algorithm on it to get the state \(\sum_{i}\alpha_i\,\ket{v_i}\,\ket{\sigma_i}\). This is discussed in \autoref{subsec:ImplementingQPE}.
            \subsubsection{Applying a gate according to a threshold}
                The goal now is to define a unitary \(\mathbf{T}_\sigma\) such that:
                ...
                where \(\mathcal{S}\) is the set of indexes so that the corresponding eigenvector has a singular value greater than the threshold \(\sigma\).
            \subsubsection{Concluding}
                ...
                We then measure the second register in the computational basis. If the outcome is \ket{1}, then we redo everything. If it is \ket{0}, then we measure the first register in the computational basis to get a product \(j\).
