...
\begin{document}
    ...
    \section{Designing a quantum circuit for a recommendation system}
        ...
        \subsection{Loading from QRAM}
            ...
            \subsubsection{Designing Quantum RAM}
                ...
                \begin{itemize}
                    ...
                    \item Qubit \ket{q_k} undergoes \(2^k\) rotations. More are actually applied on the last one, with a limit of \(2^{1+\left\lceil\log_2(p)\right\rceil}\). Hence, the total number of consecutive gates applied with this algorithm is:
                ...
                Using \(\mathbf{U}_f\), we can now have access to \ket{\theta_x}. We now want to perform to perform a rotation around the $y$-axis of angle $\theta_x$.
                ...
                % TODO: complexité de l'ensemble => Uf ?
        ...
        \subsection{Applying the Quantum Phase Estimation algorithm}
            ...
            The QPE is used to get an estimation of the singular values of \(\hat{\mathbf{P}}\). However, what the QPE outputs is an estimation of the phase of the corresponding eigenvalue. In order not to have to create the state \ket{\sigma_i} obtained after the measurement (which would be entangled anyway), we keep this ouput from the Phase Estimation and work with it, rather than with \(\ket{\sigma_i}\). Hence, the state after having applied the QPE and having uncomputed \(\mathbf{Q}\) is:
        ...
        \subsection{Separating states according to a threshold}
            ...
            \subsubsection{Maintining the result qubit in its state once the result has been found}
                ...
                \begin{figure}[ht]
                    ...
                    \end{quantikz}
                    \caption{Seoncd idea of a partial implementation of \(\mathbf{T}_\sigma\) using two ancilla qubits}
                \end{figure}
